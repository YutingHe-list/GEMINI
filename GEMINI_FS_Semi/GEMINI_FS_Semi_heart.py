...
class Trainer(object):
    def __init__(self, start_epoch=0,
                 ...
                 batch_size=1,
                 is_aug=True,
                 labeled_train_dir='data/Heart/train/labeled',
                 unlabeled_train_dir='data/Heart/train/unlabeled',
                 labeled_test_dir='data/Heart/test/labeled',
                 checkpoint_dir='weights',
                 result_dir='results',
                 model_name='GEMINI_FS_Semi_heart'):
        ...
        train_dataset = DatasetFromFolder3D_train(self.labeled_train_dir, self.unlabeled_train_dir, self.n_classes)
        self.dataloader_train = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
        test_dataset_seg = DatasetFromFolder3D_test_seg(self.labeled_test_dir, self.n_classes)
        self.dataloader_test_seg = DataLoader(test_dataset_seg, batch_size=1, shuffle=False)
        test_dataset_reg = DatasetFromFolder3D_test_reg(self.labeled_test_dir, self.n_classes)
    ...
    def test(self):
        ...
        for i, (mi, ml, fi, fl, name1, name2) in enumerate(self.dataloader_test_reg):
            ...
            if name1 is not name2:
                ...
                flow = flow.data.cpu().numpy()[0].astype(np.float32)
                w_m_to_f = w_m_to_f.data.cpu().numpy()[0, 0].astype(np.float32)
                w_label_m_to_f = np.argmax(w_label_m_to_f.data.cpu().numpy()[0], axis=0).astype(np.int8)
